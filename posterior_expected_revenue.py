...
X, y_true = make_classification(n_samples=N, n_features=k, random_state=123)

print(X.shape)
y_true.shape

X_train, X_test, y_train, y_test = train_test_split(X, y_true, shuffle=False, test_size=0.1)
...
class model_value:
    ...
    def __init__(self, reward : float = 1, penalty : float = 1, 
    ...
    def posterior_inference(self, prior_para = np.array([2]*4), MCsim : int = 10**4, seed : int = 123)-> pd.DataFrame:
        """
...
# Call:
m = model_value(reward = 1, penalty = 1, avg_tp_gain = 600, avg_fp_gain = 0, avg_tn_gain = 0, avg_fn_gain = -600)

#m.calc_avg_gain(y_pred_train, y_train, subro_amounts_trunc)    # in case you have the subro amounts
m.calc_avg_gain(y_pred_train, y_train)

posterior_samples = m.posterior_inference(MCsim = 10**4, prior_para = np.array([1]*4))
