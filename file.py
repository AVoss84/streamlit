...
class CSVService():
    def __init__(self, path="", delimiter="\t", encoding="UTF-16", schema_map=None, root_path=glob.UC_DATA_DIR,
                 **kwargs):
...
class XLSXService():
    def __init__(self, path="", sheetname="Sheet1", root_path=glob.UC_DATA_DIR, schema_map=None, **kwargs):
        self.path = os.path.join(root_path, path)
...
class PickleService():
    def __init__(self, path = "", root_path = glob.UC_DATA_DIR, id_col = None, schema_map = None, df=True, **kwargs):
        if root_path:
            self.path = os.path.join(root_path, path)
        else:
            self.path = path
...
#         table = sqlalchemy.Table(table_name, metadata, autoload=True, autoload_with=self.conn)
#         columns = table.c
#         dtypes = {}
#         for c in columns:
...
#             with open(os.path.join(self.root_path, self.path, self.source_system +filename) , 'r') as stream:
#                 try:
#                     my_yaml_load = yaml.safe_load(stream)
#                     if self.verbose:
#                         print("Read: "+self.root_path+self.path+self.source_system+filename)
#                     return my_yaml_load    
...
#             with open(os.path.join(self.root_path, self.path, self.source_system +filename), 'w') as outfile:
#                 try:
#                     yaml.dump(X, outfile, default_flow_style = False, **self.kwargs)
#                     if self.verbose:
#                         print("Write to: "+self.root_path+self.path+self.source_system+filename)
#                     try:
#                         # Set rwx for owner, group, others:
#                         os.chmod(self.path, stat.S_IRWXG | stat.S_IRWXU | stat.S_IRWXO)
#                     except Exception as e:
